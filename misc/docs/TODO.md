...
[x] **Dwarf Mail log substitution: `${count}` missing (BGA #233796).** FIXED. [Op_preventDamage::getExtraArgs](../../modules/php/Operations/Op_preventDamage.php#L66) now `array_merge`s `parent::getExtraArgs()` so the prompt `'Prevent ${count} of ${max} damage?'` resolves both placeholders client-side. Relaxed the exact-equality assertion in `Op_preventDamageTest` and flipped `Campaign_DwarfMailLogTest::testPreventDamagePromptCountPlaceholderResolves` to assert the fixed behavior.
