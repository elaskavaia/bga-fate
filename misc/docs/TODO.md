...
**Bug triage last checked:** 2026-07-23 08:32 EDT - only triage BGA reports created/updated after this time; bump this line (to `date` at run start) after each triage run.
...
[ ] **Health count not refreshing on client after damage (BGA #233794).** LEAD - client-side, not confirmable with the server test harness. After a hero takes damage, the life/health count on the miniboard only updates after a move or F5; the damage notification isn't re-rendering the health pill (server state is correct - F5 shows the right value). Suspect the client damage/health-notif handler in `src/` doesn't refresh the health display element. Needs a manual/harness client check, not a Campaign test. Reporter Vigilante8, table 887519986. Report left Open pending a client-side look.
[ ] **Dwarf Mail log substitution: `${count}` missing (BGA #233796).** CONFIRMED by red test `Campaign_DwarfMailLogTest::testPreventDamagePromptCountPlaceholderResolves`. Prompt [Op_preventDamage.php:63](../../modules/php/Operations/Op_preventDamage.php#L63) `'Prevent ${count} of ${max} damage?'` fails client `string.substitute` on `count`: [Op_preventDamage::getExtraArgs](../../modules/php/Operations/Op_preventDamage.php#L66) returns only `["max" => ...]`, overriding [CountableOperation::getExtraArgs](../../modules/php/OpCommon/CountableOperation.php#L53) which supplies `count`/`mcount`. Effect works; log-only error, F5 doesn't fix (it's in the log). FIX: `return array_merge(parent::getExtraArgs(), ["max" => $this->getCurrentDamage()])`; also relax the exact-equality assertion in `Op_preventDamageTest` (currently asserts `getExtraArgs() == ["max"=>5]`, which locked in the bug).
[ ] **Mining Equipment never clickable / unusable (BGA #233793).** CONFIRMED by red test `Campaign_MiningEquipmentTest::testMiningEquipmentClickableAdjacentToMountain`. `card_equip_4_17` Mining Equipment has `r=spendDurab:custom` ([card_equip_material.csv:54](../../misc/card_equip_material.csv#L54)); the `custom` gain half is the unimplemented stub [Op_custom](../../modules/php/Operations/Op_custom.php#L14) (always `ERR_NOT_APPLICABLE`), and [Op_paygain](../../modules/php/Operations/Op_paygain.php#L41) marks the whole op void when any delegate is void, so [CardGeneric::canBePlayed](../../modules/php/Model/CardGeneric.php#L88) returns false and `Op_useCard` never lists the card - unclickable, no error (matches report). FIX: implement the effect (a `c_mining` custom op like `Op_c_orebiter`, or a data-driven `adj(mountain)` gold gain: 2 gold if adjacent to >=1 mountain, 3 gold if adjacent to 3) so the gain half is non-void. Sibling risk: any *active* card with `X:custom` as a paygain gain-half fails the same silent way - only Mining Equipment qualifies today (Sindra's Amulet shares it but is `#`-disabled).
