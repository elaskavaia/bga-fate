...
[x] **Health count not refreshing on client after damage (BGA #233794).** FIXED (client-side, NOT browser-verified - Victoria please confirm on Studio). Root cause: the hero figure's remaining-health readout is the `.stats_attack` box (`counter_remhealth = maxHealth - damage`), refreshed only by `refreshAttackStat`, which for heroes fired only on `updateToken` (a move) - hence "updates after a move or F5". The monster damage path already called `refreshAttackStat` in [Game.ts updateBucketCount](../../src/Game.ts#L607); the hero branch did not. FIX: added `this.refreshAttackStat(heroMatch[1])` in the hero red-crystal branch so the health box (and heal) refresh live. No harness/jstest coverage for this display path; needs a manual Studio check (reporter Vigilante8, table 887519986).
...
[x] **Mining Equipment never clickable / unusable (BGA #233793).** FIXED. Implemented [Op_c_mining](../../modules/php/Operations/Op_c_mining.php) (mirrors `Op_c_orebiter`, reuses `Game::countAdjMountains`): gain 2 gold [XP] adjacent to >=1 mountain, 3 gold adjacent to >=3, voids with no adjacent mountain. Changed `card_equip_4_17` r from `spendDurab:custom` to `spendDurab:c_mining` and registered the op. Tests in `Campaign_MiningEquipmentTest` (clickable, not-clickable-off-mountains, both gold tiers). NOTE (Victoria): the card prints two `[DAMAGE]` lines; I modeled it as one auto-selecting-best-tier gain rather than the `(A)/(B)` choice convention used by Ring of Au - functionally equivalent for an optimal player. Sibling risk from the report stands: any *active* card with `X:custom` as a paygain gain-half fails the same silent way (only Sindra's Amulet shares it today, and it is `#`-disabled).
