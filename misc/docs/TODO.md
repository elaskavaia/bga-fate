...
[ ] **Speedy Attack softlock: offered when it's the only card in hand (BGA #233685).** CONFIRMED by red test `Campaign_SpeedyAttackSoftlockTest::testSpeedyAttackNotOfferedAsOnlyCardInHand`. Speedy Attack (`card_event_2_33` / `_3_33`, r=`discardEvent:actionAttack`, "Discard another card from hand to perform an attack action") over-counts affordability: [Op_discardEvent::getPossibleMoves](../../modules/php/Operations/Op_discardEvent.php#L40) counts the event card itself, so with Speedy Attack as the only hand card the probe sees 1 >= 1 and offers it; at execution [Card::useCard](../../modules/php/Model/Card.php#L281) discards the event card first, leaving `discardEvent` with no target, non-skippable -> softlock, Undo can't roll back. Same class as Inspire Defense #233418 but a different op; 9365cdc (spendManaAny) does NOT cover it. FIX: exclude the card being played (`getDataField("card")`) from the count/candidates in `Op_discardEvent::getPossibleMoves`; guard empty `card` (op also runs standalone for hand-limit discards). The red test turns green on fix.
