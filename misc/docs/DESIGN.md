...
`Op_moveStep` survives only as a backward compatibility shim extending `Op_move`: a table already in progress when the merge deployed can hold a "moveStep" row in its machine queue, and an op type with no class fails to instantiate. It maps the old "budget" data field onto the count and forces step mode; resolving one hop re-queues a plain "move", so the type dies out after a single step. Never queue it from new code.
...
8. **Undo support**: Use existing DbMultiUndo infrastructure. Allow undo within a turn (before confirming end of turn). Savepoints are written only at barriers - game setup (written IMMEDIATELY in the setup request via `customUndoSavepoint(..., immediate: true)`, because the deferred flush never runs there and in solo nothing re-anchors until turn 2; see BGA #238872), the seat switch in `OpMachine::dispatchOne` (multiplayer), and immediately after information is revealed (event draw, attack dice, monster die). A barrier drops that player's earlier snapshots and is anchored *after* the reveal, so a roll or a drawn card can never be rewound, only the decisions taken since. Snapshots are per player and normally taken at end of request (see `Game::customUndoSavepoint` / `doCustomUndoSavePoint`); the `immediate` flag is reserved for spots where nothing random or interactive sits between the call and the intended rewind point.
...
2. **Orebiter - you attack the mountain, not a monster, and it consumes your attack action**: "You may attack adjacent mountain areas. For each damage dealt, gain 1 gold [XP]." Clarified by the designer:
   ...
   - No mining from inside Grimheim (RULES.md Grimheim isolation - no terrain interaction across the walls, ruled by Victoria): both `Op_c_orebiter::getPossibleMoves()` and `countAdjMountains` bail out when the hero's hex is in Grimheim, even on the border hex next to a mountain. Whether the isolation also zeroes `countAdjMonsters`/`countAdjLegends` is an open question tracked in BUG_TRIAGE.md.
