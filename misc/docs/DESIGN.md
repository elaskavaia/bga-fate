...
8. **Undo support**: Use existing DbMultiUndo infrastructure. Allow undo within a turn (before confirming end of turn). Savepoints are written only at barriers - turn start, and immediately after information is revealed (event draw, attack dice, monster die). A barrier drops that player's earlier snapshots and is anchored *after* the reveal, so a roll or a drawn card can never be rewound, only the decisions taken since. Snapshots are per player and taken at end of request (see `Game::customUndoSavepoint` / `doCustomUndoSavePoint`).
