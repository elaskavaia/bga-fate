...
class Campaign_AlvaEventTest extends CampaignBaseTest {
    ...
    /**
     * BGA #233685 softlock: Speedy Attack played as the ONLY card in hand. Card::useCard discards
     * the event card itself before its effect runs, so the discardEvent cost was left with an empty
     * hand and no card to pay, while the pre-play affordability probe (CardGeneric::canBePlayed ->
     * noValidTargets) counted the event card itself as a discardable candidate. The card was
     * offered anyway and the machine stuck in a live discardEvent state with zero valid moves,
     * which Undo cannot roll back. Op_discardEvent::getPossibleMoves now excludes the card being
     * played (getDataField("card")), matching the "another card" wording.
     */
    public function testSpeedyAttackNotOfferedAsOnlyCardInHand(): void {
        ...
        $this->assertNotValidTarget($speedy, "Speedy Attack must be hidden when it is the only card in hand (no other card to discard)");
    }
}
