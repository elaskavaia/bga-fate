...
export class Game extends Game1Tokens {
  ...
  getCharHex(charId: string): string | null {
  ...
  getMonsterDieAttackBonus(): number {
    if ($("die_monster")?.parentElement?.id !== "display_monsterturn") return 0;
  ...
  getEffectiveMonsterAttack(charId: string, base: number): number {
    let attack = base;
  ...
  refreshAllMonsterAttackStats(): void {
  ...
  private getMonsterAttackRange(tokenId: string, faction: string): number {
    if (tokenId === "monster_legend_4_2") return 3;
  ...
  buildLegendTooltip(tokenInfo: TokenDisplayInfo) {
    ...
    const specialAbility: Record<string, string> = {
      ...
      "2_1": _("On arrival, places skeletons in all unoccupied areas adjacent to the Temple Ruins."),
      "2_2": _("As her attack, deals 1 unpreventable damage to all heroes everywhere."),
      "3_2": _("Attacks twice; each rune counts as two hits."),
